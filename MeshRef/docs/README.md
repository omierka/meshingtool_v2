...
* `load_target_mesh` reads `Coarse_meshDir/Mesh.tri` from the CLI-specified folder. After loading, it inspects `setup.e3d` for `HexMesher=HollowCylinder` or `HexMesher=FullCylinder` and toggles the cylindrical template insertion (which now blends toward the box mapping near the axis) accordingly.
...
* `-f, --folder <dir>` (mandatory) – working folder; expects `Coarse_meshDir/Mesh.tri`, `area.txt`, and (for inflow-aware refinement) `setup.e3d`.
...
3. Read `area.txt` into `hex_mesh(0)%monitor`, smooth the levels via `enforce_refinement_levels`, and then boost every inflow boundary element (as defined by `setup.e3d`) by +1 level, capped at the requested recursion depth. The boost uses the shared `setupe3dfile_reader`/`bc_treatment` helpers to classify inflow faces and only touches elements that lie on the domain boundary (fewer than six neighbors); the smoothing step is repeated afterwards to keep the level differences within ±1. If `setup.e3d` is missing or declares zero inflows the boost phase is skipped.
